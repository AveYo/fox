...
::# v9 retired dec_url64; v8 MSEPath fallback; v7 no more cmd flash ;) v6 PoS Defender so went vbscript-less too
...
@echo off & title ChrEdgeFkOff || AveYo 2022.08.23
...
reg add "%IFEO%\ie_to_edge_stub.exe\0" /f /v Debugger /d "%Headless_by_AveYo% \"%ProgramData%\ChrEdgeFkOff.cmd\"" >nul
reg add "%IFEO%\msedge.exe" /f /v UseFilter /d 1 /t reg_dword >nul
reg add "%IFEO%\msedge.exe\0" /f /v FilterFullPath /d "%MSE%" >nul
reg add "%IFEO%\msedge.exe\0" /f /v Debugger /d "%Headless_by_AveYo% \"%ProgramData%\ChrEdgeFkOff.cmd\"" >nul
if "%CLI%" neq "" exit /b
echo;& %<%:f0 " ChrEdgeFkOff V9 "%>>% & %<%:2f " INSTALLED "%>>% & %<%:f0 " run again to remove "%>%
timeout /t 7
exit /b
...
if "%CLI%" neq "" exit /b
echo;& %<%:f0 " ChrEdgeFkOff V9 "%>>% & %<%:df " REMOVED "%>>% & %<%:f0 " run again to install "%>%
timeout /t 7
exit /b
